...
def about(request):
    ...
    df = pd.read_csv("cvk/Training.csv")
    ...
    # TRAINING DATA tr --------------------------------------------------------------------------------
    tr = pd.read_csv("cvk/Testing.csv")
    tr.replace(
        {'prognosis': {'Fungal infection': 0, 'Allergy': 1, 'GERD': 2, 'Chronic cholestasis': 3, 'Drug Reaction': 4,
                       'Peptic ulcer diseae': 5, 'AIDS': 6, 'Diabetes ': 7, 'Gastroenteritis': 8,
                       'Bronchial Asthma': 9, 'Hypertension ': 10,
    ...
    # -----------------------------------------------------
    psymptoms = [request.POST['arr1'], request.POST['arr1'], request.POST['arr2'], request.POST['arr3'],
                 request.POST['arr4']]
    ...
    psymptoms = [request.POST['arr'], request.POST['arr1'], request.POST['arr2'], request.POST['arr3'],
                 request.POST['arr4']]
    ...
    ins = feedback(patient=request.POST['fname'], name=p1, email=p2, subject=p3)
    ins.save()
    data1 = feedback.objects.all().last()
    context = {'data': data1}
    return render(request, "about.html", context)



def retrieveInf(request, sklearn=None):
    return render(request, "retrieveInf.html")
